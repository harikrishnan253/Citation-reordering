...
if not os.path.exists(file_path):
    ...
    print("Dual-format validator (supports both styled and superscript citations)\n")
    try:
        ...
        if results.get('citation_sequence'):
            print(f"\nCitation Sequence (appearance order): {results['citation_sequence'][:20]}...")
        
        print(f"\nRenumber Status: {results.get('renumber_attempt', {}).get('renumbered', False)}")
        
        if results.get('renumber_attempt', {}).get('renumbered'):
            format_used = results.get('renumber_attempt', {}).get('format', 'unknown')
            print(f"\n✓ Citations renumbered using {format_used} format")
            ...
            if renumber_map:
                ...
                for i, (old, new) in enumerate(sorted(renumber_map.items())[:10]):
                    print(f"  {old} → {new}")
                if len(renumber_map) > 10:
                    print(f"  ... and {len(renumber_map) - 10} more mappings")
        else:
            error_msg = results.get('renumber_attempt', {}).get('message', 'Unknown error')
            print(f"\n✗ Renumbering failed: {error_msg}")
